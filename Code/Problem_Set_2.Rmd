...
``` 
Indexing is ok, but it cannot be automated and requires you, the human, to read the data and determine which cells to change. That is possible (but hard) in a dataset of 46 rows, but what if you had 46,000 rows? A better option would be to use `which()` to determine the rows that meet certain conditions. See the key for an example.
...
``` 
You could save yourself a lot of trouble by using group_by to include BOTH costume and type. That would eliminate the need to then join the datasets together again below.
...
```
Again, you are using indexing, which is good, but in a way that requires a human to find the correct rows, which is inefficient at best and likely to result in mistakes in a larger data set.
...
```
You didn't adjust the x and y labels to match.
